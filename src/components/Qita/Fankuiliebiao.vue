<template>
  <div class="index">
    ...
    <div class="nav2">
      ...
      <div class="myTable">
        <vxe-table :data="tableData">
          ...
          <vxe-table-column field="content" title="意见内容"></vxe-table-column>
          <vxe-table-column field="image" title="反馈图">
          ...
          <vxe-table-column field="created_at" title="反馈时间"></vxe-table-column>
          <vxe-table-column field="replay" title="平台回复"></vxe-table-column>
          <vxe-table-column title="操作状态" width="100">
            <template slot-scope="scope">
              <div class="flex">
                <el-button size="small" @click="huifu(scope.row)" type="text">回复</el-button>
              </div>
            </template>
          </vxe-table-column>
        </vxe-table>
        <el-pagination
          ...
          :current-page="this.fankuiliebiaoPage"
          :page-size="10"
    ...
    <!-- 平台回复 -->
    <el-dialog title="平台回复" :visible.sync="dialogVisible" width="600px" :before-close="handleClose">
      <div class="nav2">
        <div class="myForm">
            ...
            <el-row>
              <el-col :span="22">
                <el-form-item label="回复内容：">
                  <el-input type="textarea" size="small" v-model="ruleForm.replay"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            <el-form-item>
...
export default {
  computed: {
    ...mapState(["fankuiliebiaoPage", "fankuiliebiaoPageSize"])
  },
  watch: {
    fankuiliebiaoPage: function(page) {
      this.$store.commit("fankuiliebiaoPage", page);
      this.getData();
    },
    fankuiliebiaoPageSize: function(pageSize) {
      this.$store.commit("fankuiliebiaoPageSize", pageSize);
      this.getData();
    }
  },
  data() {
  ...
  methods: {
    async getData() {
      const res = await this.$api.feedbacks({
        limit: this.fankuiliebiaoPageSize,
        page: this.fankuiliebiaoPage
      });
    ...
    // 提交
    async submitForm() {
      const res = await this.$api.upDatefeedbacks(
        {
          replay: this.ruleForm.replay
        },
        this.id
      );
      console.log(res.data);
    ...
    handleSizeChange(val) {
      ...
      this.$store.commit("fankuiliebiaoPageSize", val);
    },
    handleCurrentChange(val) {
      console.log(`当前页: ${val}`);
      this.$store.commit("fankuiliebiaoPage", val);
    }
  }
};
</script>
