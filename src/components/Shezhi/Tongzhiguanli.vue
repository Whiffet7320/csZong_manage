<template>
  <div class="index">
    ...
    <div class="nav2">
      ...
      <div class="tit1">
        <el-button @click="addWenzhang" size="small" type="primary" icon="el-icon-plus">发布通知</el-button>
      </div>
      <div class="myTable">
          ...
          <el-row>
            <el-col :span="20">
              <el-form-item label="标题：">
                <el-input size="small" placeholder="请输入标题" v-model="lhForm.title"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="20">
              <el-form-item label="内容：">
                <el-input size="small" :rows="5" type='textarea' placeholder="请输入内容" v-model="lhForm.content"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
...
export default {
  ...
  created() {
    this.$store.commit("guanjianciPage", 1);
    this.getData();
  },
  methods: {
    async getData() {
    ...
    async AddOnSubmit() {
      // 添加
      if (this.isAdd) {
        const res = await this.$api.addMessage({
          user_id: '',
    ...
    // 删除
    async tabDel(row) {
      const res = await this.$api.delMessage(row.id);
      if (res) {
        this.$message({
          message: '删除成功',
          type: "success"
