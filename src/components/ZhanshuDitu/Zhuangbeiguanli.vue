    ...
    <div class="nav2">
            ...
            <el-col :span="20">
              <el-form-item label="类型：">
                <el-radio-group v-model="formInline.rad1" size="small" @change="changRad1">
                  <el-radio-button v-for="item in fenleiData" :key="item.id" :label="item.id">{{item.title}}</el-radio-button>
                </el-radio-group>
              </el-form-item>
            </el-col>
          </el-row>
      ...
      <div class="myTable">
        <vxe-table :data="tableData">
          ...
          <vxe-table-column field="title" title="装备名"></vxe-table-column>
          <vxe-table-column field="image" title="装备封面图">
          ...
          <vxe-table-column field="image" title="装备图">
            <template slot-scope="scope">
              <el-image
          ...
          <vxe-table-column field="classify_name" title="类型"></vxe-table-column>
          <vxe-table-column field="content" title="描述"></vxe-table-column>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.equip_list({
        pagesize: this.lunbotuliebiaoPageSize,
        page: this.lunbotuliebiaoPage,
        classify_id:this.formInline.rad1
      });
    ...
    async getFenleiData(){
      const res = await this.$api.equip_classify()
