    ...
    <div class="nav2">
      ...
      <div class="myTable">
        ...
        >
          ...
          <vxe-table-column field="title" title="战术标题"></vxe-table-column>
          <vxe-table-column field="pic" title="战术封面地图">
...
export default {
  ...
  methods: {
    async getData() {
      this.loading = true;
      this.activeName = this.tabShopIndex;
      const res = await this.$api.tactics_list({
    ...
    async tabDel(row) {
      const res = await this.$api.del_tactics({ id: row.id });
