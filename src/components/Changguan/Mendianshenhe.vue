    ...
    <div class="nav2">
      ...
      <div class="myTable">
        ...
        >
          ...
          <vxe-table-column field="store_name" title="门店名称"></vxe-table-column>
          <vxe-table-column field="store_pics" title="门店图">
            <template slot-scope="scope">
              <el-image
                :src="scope.row.store_pics"
                :preview-src-list="[scope.row.store_pics]"
                fit="fill"
          ...
          <vxe-table-column field="store_provincename" title="省份"></vxe-table-column>
          <vxe-table-column field="store_cityname" title="市"></vxe-table-column>
          <vxe-table-column field="store_areaname" title="区"></vxe-table-column>
          <vxe-table-column field="store_address" title="详细地址"></vxe-table-column>
          <vxe-table-column field="recommend_usernickname" title="用户名"></vxe-table-column>
          <vxe-table-column field="recommend_userfacepic" title="用户头像">
            <template slot-scope="scope">
              <el-image
                :src="scope.row.recommend_userfacepic"
                :preview-src-list="[scope.row.recommend_userfacepic]"
                fit="fill"
                style="width: 40px; height: 40px;margin:0 4px"
              >
                <div slot="error" class="image-slot">
          ...
          <vxe-table-column field="create_time" title="申请时间"></vxe-table-column>
          <vxe-table-column title="操作" width="120">
            <template slot-scope="scope">
              <div class="flex">
                <el-button size="small" type="text" @click="tongguo(scope.row)">通过</el-button>
                <el-button size="small" type="text" @click="jujue(scope.row)">拒绝</el-button>
              </div>
            </template>
          </vxe-table-column>
        </vxe-table>
...
export default {
  ...
  methods: {
    async getData() {
      this.loading = true;
      const res = await this.$api.storesettled_list({
    ...
    async tongguo(row){
      const res = await this.$api.storesettled_setstatus({
        id:row.id,
        status:1
      })
