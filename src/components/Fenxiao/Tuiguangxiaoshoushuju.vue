    ...
    <div class="nav2">
      ...
      <div class="myTable">
        <vxe-table :data="tableData">
          ...
          <vxe-table-column field="goods_count" title="数量"></vxe-table-column>
          <vxe-table-column field="orders_amount" title="总支付"></vxe-table-column>
...
export default {
  ...
  methods: {
    async getData() {
      const res = await this.$api.recommendsale_list({
        pagesize: this.lunbotuliebiaoPageSize,
        page: this.lunbotuliebiaoPage,
        recommend_userid: this.formInline.recommend_userid,
        beginday: this.formInline.time[0],
        endday: this.formInline.time[1]
      });
      ...
      this.tableData.forEach(ele => {
        ele.myHierarchy_types = ele.hierarchy_types == '1' ? '一级' : ele.hierarchy_types == '2' ? '二级' : '居间'
      });
    },
    async getFenleiData() {
