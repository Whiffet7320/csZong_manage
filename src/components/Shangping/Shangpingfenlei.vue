        ...
        >
          ...
          <vxe-table-column field="pic" title="分类图标">
            <template slot-scope="scope">
              <el-image :src="scope.row.pic" fit="fill" style="width: 40px; height: 40px">
          ...
          <vxe-table-column field="store_name" title="名店名称"></vxe-table-column>
          <vxe-table-column field="proportion_ratio" title="抽成比例(%)"></vxe-table-column>
          ...
          <el-row>
            <el-col :span="10">
              <el-form-item label="抽成比例">
                <el-input size="small" v-model="addForm.ratio"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <!-- <el-row>
...
export default {
  ...
  methods: {
    async getData() {
      this.loading = true;
      const res = await this.$api.goodsclassify_list({
        page: 1,
        pagesize: 1000
      });
      console.log(res);
      ...
      this.tableData.forEach(ele => {
        ele.is_showKG = ele.is_status == 1 ? true : false;
      });
      this.loading = false;
    },
    // 开关（显示/隐藏）
    async changeKG(row) {
      console.log(row);
      const res = await this.$api.setstatus_goodsclassify({
        id: row.id,
        is_status: row.is_showKG == true ? "1" : "0"
      });
    ...
    async tabDel(row) {
      console.log(row);
      const res = await this.$api.del_goodsclassify({id:row.id});
      if (res.data.result == 1) {
        this.$message({
          message: res.msg,
          type: "success"
    ...
    AddOnSubmit(formName) {
      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          if (this.addForm.id == "") {
            // 新增
            const res = await this.$api.add_goodsclassify({
              title: this.addForm.name,
              pic: this.addForm.pic,
              ratio: this.addForm.ratio,
              is_status: this.addForm.is_show
            });
            if (res.data.result == 1) {
              this.$message({
                message: res.msg,
            ...
            // 编辑
            const res = await this.$api.edit_goodsclassify(
              {
                title: this.addForm.name,
              pic: this.addForm.pic,
              ratio: this.addForm.ratio,
